...
class trainingData(object):
    ...
    def preprocess(self,target, unique = 3):
        ...
        self.out_mapper = sklearn.preprocessing.RobustScaler(with_centering=True,copy=False)
...
td.preprocess('Sales',unique=3)

#td.expt("hello.csv")
#td = trainingData("temp.csv")
#td.preprocess('MEDV')

import final

nn = final.nnnet(td.trainX.astype(np.float32),td.trainY.astype(np.float32),\
                 td.testX.astype(np.float32),td.testY.astype(np.float32),\
                 8,0.0,0.0)


print sklearn.metrics.mean_squared_error(td.trainY,nn.Blah())


#temp = td.out_mapper.inverse_transform(nn.Blah())
#print temp